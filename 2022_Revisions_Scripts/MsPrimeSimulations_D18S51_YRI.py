...
model = msprime.SMM(lo=28, hi=41)
STR_mu = str(0.00220917) #D18S51
SNP_mu = str(2e-8)
...
def save_genotypes_byPop(strs, snps, num_samples, s_len, rep):
	...
	D18S51_1 = alleles[str_info.genotypes]

	# We add adjacent rows for diploid genotypes (taking the average of the STR)
	D18S51 = (D18S51_1[::2] + D18S51_1[1::2])/2

	# Subset Admixed (first samples) and European Samples 
	D18S51_Admx = D18S51[:num_samples]
	D18S51_EUR = D18S51[num_samples:]
	...
	np.savetxt(adx_str_fname, D18S51_Admx, fmt='%1.2f')
	np.savetxt(adx_snp_fname, snps_Admx, fmt='%1d')
	np.savetxt(eur_str_fname, D18S51_EUR, fmt='%1.2f')
